...
// Watch attaches one generic consumer to a scheduler channel. WebSocket
// viewers and durable tracking runs use the same primitive, so the scheduler
// never needs to know why a consumer exists.
func (s *Scheduler) Watch(channel string, params realtime.SubscribeParams, send func(realtime.Event)) (func(), error) {
	if params == nil {
		params = realtime.SubscribeParams{}
	}
