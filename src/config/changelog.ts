...
export const CHANGELOG_RECENT: ChangelogEntry[] = [
  {
    ...
    summary: {
      en: "Search gains keyboard navigation, clear and retry actions, and better city matching. Map, unit and alert preferences are grouped and saved together, with more room in phone landscape. Traffic filters offer direct choices, clear-all and accessible menus. Here stays centered on each GPS update. Loading indicators lose their card backdrop. Numeric detail tiles share per-digit transitions while preserving their formatting. Flight readings retain independent unit choices; weather pressure and ATC frequencies include their units. Fixes stale cached app icons.",
      zh: "搜索支持键盘选择、清除与重试，城市名称匹配更准确。地图、单位和提醒设置分组呈现并统一保存或取消，手机横屏也有更多操作空间；流量筛选支持直接选择、一键清除和键盘菜单。Here 始终跟随最新 GPS 坐标居中，加载动画去掉卡片底板。详情数字 tile 统一逐位跳动并保留原有格式；航班各项读数独立切换并保留单位选择，天气气压和 ATC 频率补齐单位，并修复旧缓存仍显示旧图标的问题。",
    },
