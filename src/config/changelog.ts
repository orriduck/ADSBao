...
export const CHANGELOG_RECENT: ChangelogEntry[] = [
  {
    ...
    summary: {
      en: "Airport maps retain aircraft and nearby airports within an 80 NM airport-centred area, where you can pan within the boundary and recenter from Map range. Flight tracking locks its live map view, keeps the watched aircraft's full current-leg trail visible, and keeps its trace, aircraft marker, route-to-destination line, and centre anchored to the same inferred position for a full minute during a brief feed gap. Position-source cooldowns now isolate tracked-flight lookups from airport traffic. Regional traffic refreshes every three seconds: eligible sources race once, the first usable snapshot stays pinned for stable aircraft positions, and a new race occurs only if that source becomes unavailable. A service-wide three-second traffic lane prevents overlapping maps from rate-limiting that pinned source. Selected airport runway previews remain open while nearby context refreshes. Completed map-setting saves appear as full, unobstructed notices.",
      zh: "机场地图会保留以机场为中心、80 海里范围内的飞机与附近机场；你可以在该范围内移动视野，并从地图范围菜单回到机场中心。航班追踪页锁定实时地图视图，始终保留被追踪飞机当前航段的完整轨迹，并在短暂数据缺口中让航迹、飞机标记、到目的地的路径和地图中心继续锚定于同一推断位置一分钟。位置源冷却现在会与航班追踪查询隔离。区域流量每三秒刷新：可用来源只在首次竞速，最先返回的有效快照会被固定使用以保持飞机位置稳定；只有该来源不可用时才会重新竞速。服务级的三秒流量通道会阻止多个地图重叠时把固定来源推入限流。附近上下文刷新时，已选机场的跑道预览也会保持打开。地图设置保存完成后则以完整、不被遮挡的提示显示。",
    },
    highlights: [],
  },
];
