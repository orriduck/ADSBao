...
export const CHANGELOG_RECENT: ChangelogEntry[] = [
  {
    ...
    summary: {
      ...
      zh: "机场、航班与首屏统一采用 36px 导视边，以清晰层级、克制的追踪色、图形化数据来源边栏、对齐的指标基线与中性实时升降标识（含无障碍方向标注）提升扫读效率；机场流量源恢复期间也会继续显示可用的 SSE 缓存航迹。目录行现以 IATA 代码领衔；所有预览卡现在在桌面和移动端共用同一柔和圆角及可见的中性身份 rail，移动端工具栏以淡阴影浮起，加载动画固定居中于地图区。目录与搜索结果行的 rail 现连续贯通，不再有间隙或分隔线切断 rail 列。全屏移动端详情面板不再以桌面专用的侧边线框住 logo 顶栏，Near me rail 也改为直角衔接周围导视列；About 与机制页都将组标题收进更平的连续列表。",
    },
    highlights: [],
  },
  {
