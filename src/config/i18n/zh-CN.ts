...
const zhCN = {
  ...
  mechanism: {
    ...
    items: {
      realtimeBackbone: {
        ...
        signal: "新的坐标绝不从缓存重放",
        body:
          "追踪飞机先进行一次同源、no-store 的位置查询；随后 nearby Server-Sent Events 流把焦点飞机、附近交通和中性状态作为独立的归一化更新持续送达。",
        flow: {
          browser: "打开航班",
          socket: "新鲜首帧",
          scheduler: "SSE 附近上下文",
        ...
        details: {
          connect:
            "机场和 here 页面使用两位小数坐标 key。飞机追踪先解析新的呼号位置，再用一条呼号流接收附近上下文。",
          share:
            "相同 nearby key 在一个浏览器 tab 内共享一条 EventSource；服务端也为相同 key 共享数据源工作。",
          resume:
            "PENDING 或重连中的流没有可重放的位置：它只会唤醒一次真实的数据源轮询。具名 SSE 事件仍可在 Network 检查，地图则保持最后一个有效的可视状态。",
        },
      },
      parallelPipelines: {
        title: "并行数据管线",
      ...
      trackingTargets: {
        ...
        details: {
          aircraft:
            "飞机追踪只接受新鲜且位置时间单调递增的焦点位置，供地图、侧栏、预览卡和航迹头共用；附近列表刷新不会用旧坐标替换它。",
          airport:
      ...
      flightTrackingLifecycle: {
        ...
        signal: "新鲜首帧，随后持续上下文",
        body:
          "打开一个航班会建立干净页面，并在实时流更新前先请求一帧新的、no-store 的焦点位置。地图随后收敛到获取中、实时位置或明确的无信号/已结束卡片，绝不会用旧缓存坐标代替。",
        flow: {
          open: "打开航班",
          reload: "干净页面状态",
          acquire: "新鲜首帧",
        ...
        details: {
          reload:
            "在详情页之间跳转会干净地加载新 URL：上一个地图状态和实时连接都会被丢弃，再由新飞机建立自己的焦点实时位置。",
          states:
            "飞行地图处于获取中、已定位或终态。数据源 PENDING 时不会拿此前坐标顶替；确认没有实时位置后才显示静态的「暂无实时位置 / 信号丢失 / 航班已结束」卡片，而不是无尽转圈或不相干的兜底地图。",
          trace:
            "当前页面的飞机始终保留从主动追踪开始记录的新鲜采样点，即使点选了其他飞机也不会消失。历史航迹可以独立留存，但实时坐标不会被复用为航迹头。「完整航迹」适配起点到目的地的完整航路；「全部记录点」只适配本次主动追踪记录的点。两种视图都保留通往目的地的虚线，长航迹会压缩为有上限的渲染集合。",
        },
      },
    },
  },
  ...
  map: {
    ...
    refreshPage: "刷新当前页",
    recenter: "回到机场中心",
    nearbyBoundaryTitle: "已回到附近航班范围内",
    nearbyBoundaryDescription: "地图视野可在机场周围 {radius} 海里内移动。",
    fitTrace: "适配航迹",
    ...
    routeLoadFailedTitle: "航班追踪未能打开。",
    routeLoadFailedHint: "刷新以加载最新版本后重试。",
    routeLoadRetry: "刷新",
    flightNoPositionTitle: "暂无实时位置",
    flightNoPositionSubtext: "{callsign} 当前没有广播位置。",
    flightSignalLostTitle: "实时信号丢失",
    flightSignalLostSubtext: "已丢失 {callsign} 的实时信号。",
    flightTerminalTitle: "航班已结束",
    flightTerminalSubtext: "{callsign} 已落地或不再活跃。",
    ...
    poc: {
      ...
      selectableContextListAria: "可选择的地图要素",
      ...
      acceptanceWakeLockStatus: "屏幕常亮 · {status} · 辅助项，非门槛",
      acceptanceWakeLockUnsupported: "不支持",
      acceptanceWakeLockInactive: "未开启",
      acceptanceWakeLockPending: "正在请求",
      acceptanceWakeLockActive: "已开启",
      acceptanceWakeLockError: "请求失败",
      acceptanceWakeLockEnable: "保持屏幕常亮",
      acceptanceWakeLockDisable: "允许屏幕休眠",
      acceptanceWakeLockRetry: "重试屏幕常亮",
      acceptancePhysicalIPhone: "确认实体 iPhone",
      acceptanceThermalOk: "温度可接受",
      acceptanceThermalHot: "过热",
      acceptanceThermalLocked: "20:00 后才能确认设备温度",
      acceptanceExport: "导出报告",
      acceptanceReset: "重置报告",
      acceptanceResetConfirm: "确认重置",
      acceptanceExportState: "报告状态：{state}",
      acceptanceGateDetails: "查看门槛 · {failed} 失败 · {pending} 等待",
      acceptanceGateDuration: "运行时长",
      acceptanceGatePhysicalIPhone: "实体 iPhone",
      ...
      acceptanceGateRenderStability: "渲染稳定性",
      acceptanceGateThermal: "设备温度",
    },
  },
  mapLayers: {
    mapLabels: "地图标签",
