...
export const CHANGELOG_RECENT: ChangelogEntry[] = [
  {
    ...
    summary: {
      ...
      zh: "ADSBao 现为不含账户的直接地图与航班追踪体验。地图偏好保存在本机；新安装默认使用标准地图，开启进近引导线和拍机点，空域保持关闭。路由打开时现与实时追踪共用同一套全屏飞机、塔台和跑道循环加载动画。机场高倍率下的航站楼轮廓现仅由底图建筑渲染提供，不再叠加重复着色的 Leaflet 图层。Here 的指标卡不再继承仅供机场外链使用的拼接容器，避免移动端在空 footer 情况下被裁切或按错误高度呈现。Here 现在会在车辆跨过附近数据刷新格时保持已经完成的侧栏可见，地图与数据会在后台更新，不再反复播放加载骨架。",
    },
    highlights: [],
  },
  {
