  ...
  mechanism: {
    ...
    items: {
      ...
      flightTrackingLifecycle: {
        ...
        signal: "整页全新加载,三种状态",
        body:
          "打开一个航班会整页全新加载——拥有自己的地图和实时连接,上一个会被切断——所以在航班之间跳转绝不会残留旧地图状态。随后地图收敛到三种状态之一:正在获取信号、实时位置、或没有实时位置时的提示卡片。",
        flow: {
          open: "打开航班",
          reload: "整页加载",
          acquire: "获取信号",
        ...
        details: {
          reload:
            "在详情页之间跳转会整页 reload 到新 URL,每个飞机/机场页都是全新干净状态,没有残留地图,旧的实时连接也被切断。",
          states:
            "飞行地图处于:加载中(获取信号)、位置(以飞机为中心)、或终态——一张静态的「暂无实时位置 / 信号丢失 / 航班已结束」卡片(包括无覆盖的跨洋航段),而不是无尽转圈或一张不相干的兜底地图。",
          trace:
            "被追踪的航班绘制当前航段的航迹——更早的航段和昨天同呼号的旧航迹会被裁掉。「完整航迹」是航路视图(内部权限开启时叠加起降点之间路径);「所有记录点」与权限无关,展示本航段已记录的点。点选的飞机只显示最近航迹。",
        },
      },
    },
  },
