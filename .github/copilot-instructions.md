...
**Markers.** The aircraft layer carries three signals at once: *direction state* (color), *aircraft type* (shape), and *wake class* (size). Slow / unknown traffic is a bare dot; moving traffic without a known type is a short vector arrow oriented to the track; moving traffic with a known ICAO type designator or ADS-B emitter category is a small silhouette from the upstream icon set, tinted with the same desaturated state color via CSS mask. The base size is 18px; the ADS-B emitter A-category nudges that uniformly between roughly 16px and 20px (A1 light → 0.90×, A2 small → 0.95×, A3 large → 1.00×, A4 high-vortex → 1.05×, A5 heavy → 1.10×), so a 777 reads visibly larger than a Cessna without changing the grid. Silhouettes are read as data, not decoration — they share the dot/arrow grid, never carry their own outlines, gradients, or fills, and never appear at sizes large enough to compete with the typographic identity (callsign / route / telemetry) next to them. Anything richer — three-quarter views, photographic livery, embellished icons — stays out.
